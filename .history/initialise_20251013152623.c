...
void build_polymer(struct Parameters *p_parameters, struct Vec3D molecule_center, struct Vectors *p_vectors, int ipart)
{
    ...
    for (size_t n = ipart; n < ipart + N; n++) {
        double x = molecule_center.x + 1.54 * (n - ipart);
        // Wrap x coordinate into box
        x = fmod(x, Lx);
        if (x < 0) x += Lx;

        p_vectors->r[n].x = x;
        p_vectors->r[n].y = molecule_center.y; // already inside box
        p_vectors->r[n].z = molecule_center.z; // already inside box
    }
}
...
{
    size_t N = p_parameters->amount_mon;      // monomers per polymer
    size_t num_poly = p_parameters->num_part / N; // number of polymers
    double left_x = p_parameters->L.x / 4.0;
    double right_x = 3.0 * p_parameters->L.x / 4.0;
    double center_y = p_parameters->L.y / 2.0;
    double center_z = p_parameters->L.z / 2.0;

    for (size_t ipoly = 0; ipoly < num_poly; ++ipoly) {
        size_t ipart = ipoly * N;
        struct Vec3D center;
        // Check type of first monomer in polymer
        ...
        center.y = center_y;
        center.z = center_z;
        build_polymer(p_parameters, center, p_vectors, ipart);
    }
}
