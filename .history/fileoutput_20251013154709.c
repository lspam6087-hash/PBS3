  ...
  for (size_t i = 0; i < p_parameters->num_part; i++)
{
    ...
    // Print type as atom name (e.g., "C0", "C1", etc.)
    fprintf(fp_traj, "HETATM%5u  C%-2d UNK A   1    %8.3f%8.3f%8.3f  1.00  0.00           C\n",
            (unsigned int)i % 100000, type, rs*p_vectors->r[i].x, rs*p_vectors->r[i].y, rs*p_vectors->r[i].z);
}
